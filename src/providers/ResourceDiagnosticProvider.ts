...
import { SDK_OWNED_PREFIXES } from '../util/resourceAllowlists';
import { decodeUtf8 } from '../util/encoding';

const RES_VALUE_RE = /\bresValue\s*\(\s*["']string["']\s*,\s*["']([A-Za-z_]\w*)["']/g;
...
export class ResourceDiagnosticProvider implements vscode.Disposable {
  ...
  // Strings a build script declares with `resValue("string", "build_time", …)`:
  // they exist only in build/generated, which the index never reads.
  private _resValues = new Set<string>();
  private _resValuesLoaded: Promise<void> | undefined;
  ...
  constructor(
    ...
    this._subs = [
      ...
      vscode.workspace.onDidSaveTextDocument(doc => {
        if (/\.gradle(?:\.kts)?$/.test(doc.uri.path)) { this._resValuesLoaded = undefined; this.invalidateAll(); }
      }),
    ];
  ...
  private _scan(doc: vscode.TextDocument): void {
    ...
    if (!this._resValuesLoaded) {
      this._resValuesLoaded = this._loadResValues().then(() => this.invalidateAll());
    }

    const near = doc.uri?.path;
    ...
    for (let i = 0; i < doc.lineCount; i++) {
      ...
      while ((m = R_STRING_RE.exec(text))) {
        ...
        if (this.strings.getValue(m[1], near)) continue;
        // A qualified `com.other.lib.R.string.x` names a library's R, which
        // the index does not hold; a Firebase or Braze key is generated by the
        // SDK; a `resValue` lives in build/generated. None of them is an error
        // in code that compiles.
        if (m.index > 0 && text[m.index - 1] === '.') continue;
        if (SDK_OWNED_PREFIXES.some(p => m![1].startsWith(p))) continue;
        if (this._resValues.has(m[1])) continue;
      ...
      while ((m = R_COLOR_RE.exec(text))) {
        if (isInsideCommentOrString(text, m.index)) continue;
        if (this.colors.getValue(m[1], near)) continue;
        if (m.index > 0 && text[m.index - 1] === '.') continue;
        const d = new vscode.Diagnostic(
          new vscode.Range(i, m.index, i, m.index + m[0].length),
  ...
  private async _loadResValues(): Promise<void> {
    const found = new Set<string>();
    try {
      const files = await vscode.workspace.findFiles('**/build.gradle{,.kts}', '**/{build,node_modules,.gradle}/**', 200);
      for (const uri of files) {
        try {
          const text = decodeUtf8(await vscode.workspace.fs.readFile(uri));
          RES_VALUE_RE.lastIndex = 0;
          let m: RegExpExecArray | null;
          while ((m = RES_VALUE_RE.exec(text))) found.add(m[1]);
        } catch { /* unreadable build file */ }
      }
    } catch { /* findFiles unavailable (web) */ }
    this._resValues = found;
  }
