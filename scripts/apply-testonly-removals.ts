...
function main(): void {
  ...
  const etenduesParLabel = new Map<string, { path: string; start: number; end: number }[]>();
  for (const g of groups) (etenduesParLabel.get(g.label) ?? etenduesParLabel.set(g.label, []).get(g.label)!).push({ path: g.path, start: g.start, end: g.end });
  ...
  for (const g of groups) {
    ...
    if (plan === undefined) {
      plan = planTestCoRemoval(g.names, sources, DEFAULT_TEST_SEGMENTS, force ? undefined : liveOutside(declarations, etenduesParLabel.get(g.label) ?? []), g.allowed);
