 ...
 *
 * Quick fixes: lambda parameters and catch bindings rename to `_` (the only
 * positions where Kotlin allows it). A dead variable is deleted only when its
 * initializer is provably pure; anything that could run code keeps the
 * expression and drops the variable instead.
 */
