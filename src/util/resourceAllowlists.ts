...
/**
 * Header markers a generator writes so humans leave the file alone.
 *
 * Checked against the TOP of the file only. A `DO NOT EDIT` further down is a
 * warning about one section, not a statement about the whole file.
 */
const GENERATED_HEADER_RE =
  /\b(?:auto[- ]?generated|generated by|@generated|DO NOT EDIT|DO NOT MODIFY)\b/i;

/** How much of the file counts as its header. */
const GENERATED_HEADER_CHARS = 1500;

/**
 * True when a generator owns this file.
 *
 * Nothing in the family should report a finding here: the next build rewrites
 * the file, so acting on it is wasted, and the generator's own conventions
 * (`@SerializedName` on every enum entry, a full set of variants mirroring a
 * schema) look exactly like dead code to a textual scan. On one workspace, 165
 * files carry such a header and every enum finding inside them was wrong.
 */
export function isGeneratedSource(text: string): boolean {
  return GENERATED_HEADER_RE.test(text.slice(0, GENERATED_HEADER_CHARS));
}
