/**
 * Tests for DrawableXmlHoverProvider: hover preview (256x256 popup) of
 * `<vector>` drawables when their source XML is open. Split out of
 * DrawableXmlInlinePreviewProvider (which keeps only the always-visible
 * gutter icon, desktop-only) so this half has zero Node dependencies and
 * can register on the web too.
 */
import { describe, it, expect } from 'vitest';
import { DrawableXmlHoverProvider } from '../../src/providers/DrawableXmlHoverProvider';
...
describe('DrawableXmlHoverProvider', () => {
  const provider = new DrawableXmlHoverProvider();
